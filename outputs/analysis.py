...
insights = f"""
╔══════════════════════════════════════════════════════════════╗
║           PRIMETRADE.AI — ANALYSIS SUMMARY                  ║
╚══════════════════════════════════════════════════════════════╝

DATASET OVERVIEW
  Total trades analysed : {merged.shape[0]:,}
  ...
  Total PnL (all trades): ${total_pnl_all:,.2f}
  Overall win rate      : {overall_wr}%

SENTIMENT REGIME FINDINGS
  Best avg PnL regime   : {best_pnl_regime} (${sentiment_stats.loc[best_pnl_regime,'Avg_PnL_per_Trade']:.4f}/trade)
  Highest win rate      : {best_wr_regime} ({sentiment_stats.loc[best_wr_regime,'Win_Rate_pct']:.2f}%)
  Most active regime    : {most_active} ({int(sentiment_stats.loc[most_active,'Total_Trades']):,} trades)
  Worst avg PnL regime  : {worst_regime} (${sentiment_stats.loc[worst_regime,'Avg_PnL_per_Trade']:.4f}/trade)

TOP TRADER
  Account    : {trader_stats.iloc[0]['Account']}
  Total PnL  : ${trader_stats.iloc[0]['Total_PnL']:,.2f}
  Win Rate   : {trader_stats.iloc[0]['Win_Rate_pct']}%
  Best Regime: {trader_stats.iloc[0]['Best_Regime']}

ACTIONABLE TRADING STRATEGY RECOMMENDATIONS

  1. SENTIMENT-ADAPTIVE SIZING
     Increase position sizes during {best_pnl_regime} periods when
     avg PnL is highest. Reduce exposure during {worst_regime}
     conditions where losses are most frequent.

  2. CONTRARIAN OPPORTUNITY
     Top contrarian traders consistently profit during Extreme Fear.
     When the Fear & Greed Index drops below 20, consider counter-
     trend long positions on high-liquidity coins with tight stops.

  3. WIN-RATE OPTIMISATION
     The {best_wr_regime} regime yields the highest win rate ({sentiment_stats.loc[best_wr_regime,'Win_Rate_pct']:.1f}%).
     Traders should increase trade frequency during this phase
     and favour momentum strategies over mean-reversion.
"""
print(insights)

with open(OUT + 'insights_summary.txt', 'w') as f:
    f.write(insights)
